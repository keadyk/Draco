...
public:
  ...
  std::vector<std::vector<std::vector<std::vector<double>>>>
  get_csk_data(size_t etemp_reg, size_t interp_reg, size_t x_reg,
               size_t y_reg) const {
    ...
    std::vector<std::vector<std::vector<std::vector<double>>>> select_data(
        ppr_etemp,
        std::vector<std::vector<std::vector<double>>>(
            ppr_gin, std::vector<std::vector<double>>(
                         ppr_gout, std::vector<double>(xi_pts.size(), 0.0))));
    ...
    for (size_t a = 0; a < ppr_etemp; a++) {
      for (size_t b = 0; b < ppr_gin; b++) {
        for (size_t c = 0; c < ppr_gout; c++) {
          ...
          select_data[a][b][c] =
              csk_data[ppr_etemp * etemp_reg + a][offset + ppr_gin * x_reg + b]
                      [ppr_gout * y_reg + c];
        }
      }
    }
  ...
  std::vector<std::vector<std::vector<std::vector<double>>>>
  get_csk_data(size_t etemp_reg) const {
    // use the provided regions to determine the proper indices:
    size_t ppr_etemp = etemp_pts.size() / (etemp_breakpts.size() - 1);
    // allocate a return vector
    std::vector<std::vector<std::vector<std::vector<double>>>> select_data(
        ...
        std::vector<std::vector<std::vector<double>>>(
            ...
            std::vector<std::vector<double>>(
                gout_pts.size(), std::vector<double>(xi_pts.size(), 0.0))));
